...
function freeMemory(memory, newPage) {
    console.log(Math.floor((Math.random() * memory.pages.length + 1)))
    memory.pages = del(memory.pages, Math.floor((Math.random() * memory.pages.length )))
...
function showPagesTable(memory) {
    let str = "<table border='1'><thead><tr><td>#</td><td>Имя</td><td>Кол-во обращений</td></tr></thead><tbody>";
    
    memory.pages.forEach((page, ind) => {
        str += `<tr>`;
        str += `<td>${ind}</td><td>${page.name}</td><td>${page.queries}</td>`
        str += `</tr>`;
    })
